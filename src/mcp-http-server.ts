...
function createMcpServer(context: McpSharedContext) {
  ...
  server.registerTool("ask_agent", {
    ...
    inputSchema: {
      ...
      caller: z.string().optional().describe("Your agent name (the agent making this request). Fill this with your own name so the mesh can track who is asking."),
    },
  }, async ({ agent: agentName, question, caller }) => {
    const terminalRef = context.getTerminalRef(agentName);
    if (!terminalRef) {
      return {
        content: [{ type: "text" as const, text: `Agent "${agentName}" not found. Use list_agents to see available agents.` }],
        isError: true,
      };
    }
    ...
    const responsePromise = new Promise<string>((resolve, reject) => {
      const timeoutHandle = setTimeout(() => {
        ...
        reject(new Error(`Request to agent "${agentName}" timed out after ${REQUEST_TIMEOUT_MS / 1000}s`));
      }, REQUEST_TIMEOUT_MS);
  ...
  server.registerTool("answer_agent", {
    description:
    ...
    if (!pending) {
      return {
        content: [{ type: "text" as const, text: `No pending request found for id "${request_id}". It may have expired or already been answered.` }],
        isError: true,
      };
    }
...
export async function startMcpHttpServer(context: McpSharedContext) {
    ...
    if (req.method === "POST") {
      try {
        ...
        if (!sessionId && isInitializeRequest(jsonBody)) {
          ...
          // Capture the session ID after handling the initialize request
          transport.onclose = () => {
  ...
  return new Promise<{ port: number; stop: () => Promise<void>; getSessionCount: () => number }>((resolve) => {
